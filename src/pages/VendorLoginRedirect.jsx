...
const VendorLoginRedirect = () => {
    ...
    const saveSession = async () => {
        try {
          const vendor = await account.get();
          ...
          console.log("User logged in:", vendor);
        } catch (error) {
...
export const logout = async () => {
  try {
    ...
    */
    ...
    const doesVendorExist=async(vendorEmail)=>{
        return await database.doesVendorExist(vendorEmail);
    }
    ...
    useEffect(() => {
      const fetchUser = async () => {
        try {
          ...
          if (doesVendorExist(user.$id)){
              saveSession();  
              navigate("/vendor"); 
          } else {
              navigate("/vendor/googleLoginFailed");
          }
        } catch (error) {
          console.error("Error fetching user data:", error);
          navigate("//vendor/googleLoginFailed");  // Redirect to failure page
        }
      };
